...
export default function MembersPage() {
  ...
  const members = useProjectStore((state) => state.currentProject?.members ?? []);
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--tg-theme-bg-color)]">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {members.length === 0 ? (
          <div className="rounded-[10px] bg-[var(--tg-theme-secondary-bg-color)] p-4 text-center text-sm text-[var(--tg-theme-hint-color)]">
            Участников пока нет
          </div>
        ) : (
          members.map((member) => (
